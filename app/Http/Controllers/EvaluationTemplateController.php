...
class EvaluationTemplateController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if ($request->filled('evaluation_type')) {
            $query->where('evaluation_type', $request->string('evaluation_type'));
        }

        if ($request->has('department_id')) {
            $deptParam = $request->input('department_id');
            if ($deptParam === '' || $deptParam === null || $deptParam === 'null') {
                $query->whereNull('department_id');
            } else {
                $query->where('department_id', (int) $deptParam);
            }
        }
    ...
    public function store(Request $request): JsonResponse
    {
        $data = $request->validate([
            ...
            'evaluation_type' => ['required', 'string', Rule::in(['self', 'peer', 'manager'])],
            ...
            // Inline questions when creating a template. Questions inherit
            // evaluation_type from the parent template — they no longer carry their own.
        ...
        return response()->json(
            $this->templatePayload($template->load('department')->loadCount('questions')->load('questions.options')),
    ...
    {
        $template->load('department')->loadCount('questions')->load('questions.options');
    ...
    public function update(Request $request, EvaluationTemplate $template): JsonResponse
    {
        ...
        return response()->json($this->templatePayload($template->load('department')->loadCount('questions')));
    }
    ...
    public function questions(Request $request): JsonResponse
    {
        ...
        // evaluation_type now lives on the template — filter through the parent.
        if ($request->filled('evaluation_type') || $request->filled('evaluationType')) {
            $type = (string) ($request->string('evaluation_type') ?: $request->string('evaluationType'));
            $query->whereHas('template', fn ($q) => $q->where('evaluation_type', $type));
        }
    ...
    {
        // Questions inherit evaluation_type from their parent template — they
        // no longer accept their own evaluation_type field.
        $data = $request->validate([
    ...
    private function templatePayload(EvaluationTemplate $t, bool $includeQuestions = false): array
    {
        $payload = [
            ...
            ] : ($t->department_id ? ['id' => $t->department_id, 'name' => optional($t->department()->first())->name] : null),
